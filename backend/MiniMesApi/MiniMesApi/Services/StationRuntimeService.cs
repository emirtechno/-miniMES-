...
public sealed class StationRuntimeService(MesDbContext context) : IStationRuntimeService
{
    public async Task EnsureSeededAsync(CancellationToken cancellationToken = default)
    {
        ...
        var missing = StationCatalog.All.Where(id => !existing.Contains(id, StringComparer.OrdinalIgnoreCase)).ToList();

        var retired = existing.Where(id => !StationCatalog.Contains(id)).ToList();
    ...
    public async Task SyncWithOpenAlarmsAsync(CancellationToken cancellationToken = default)
    {
        ...
        foreach (var stationId in StationCatalog.All)
        {
